...
use CTIC\Grh\Employee\Infrastructure\Controller\EmployeeLowController;
use CTIC\Grh\Employee\Infrastructure\Form\Type\EmployeeAreaType;
use CTIC\Grh\Employee\Infrastructure\Form\Type\EmployeeLowType;
...
class CreateEmployeeLowController extends CreateResourceController
{
    /**
    ...
    ): void
    {
        ...
        $formRegistry->addType(new EmployeeLowType(EmployeeLow::class));
    }
        ...
        if(empty($command->metadata))
        {
            $command->metadata = self::getMetadata(
                ...
                array(
                    ...
                    'driver'        => '',
                    'templates'     => '',
                    'header'        => 'bajas empleados',
                    'route'         => array(
                        'root'      => 'trabajadores_baja',
                        'actions'   => array(
                            'list'  => 'listado',
                            'show'  => 'mostrar',
                            'create'    =>'crear',
                    ...
                    'classes'       => array(
                        'form'      => 'CTIC\Grh\Employee\Infrastructure\Form\Type\EmployeeLowType'
                    ),
                    'grid'          => array(
                        'actionGroups'  => array(
                            'bulk'  => array(
    ...
    public static function setDefaultCommand(ResourceControllerCommand $command): void
    {
        if(empty($command->newResourceFactory))
        {
            $command->newResourceFactory = new CreateEmployeeLow();
        }
        if(empty($command->resource))
        {
            $command->resource = EmployeeLow::class;
        }
        if(empty($command->command))
        {
            $command->command = new EmployeeLowCommand();
        }
    }

    public static function getResourceControllerClass(): string
    {
        return EmployeeLowController::class;
    }
