...
use CTIC\Grh\Fichar\Infrastructure\Form\Type\FicharType;
use CTIC\Grh\Fichar\Domain\Command\FicharCommand;
use CTIC\Grh\Fichar\Domain\Fichar;
use CTIC\Grh\Fichar\Infrastructure\Controller\FicharController;
...
class CreateFicharController extends CreateResourceController
{
    /**
    ...
    ): void
    {
        ...
        $formRegistry->addType(new FicharType(Fichar::class));
    }
        ...
        if(empty($command->metadata))
        {
            $command->metadata = self::getMetadata(
                ...
                array(
                    ...
                    'classes'       => array(
                        'form'      => 'CTIC\Grh\Fichar\Infrastructure\Form\Type\FicharType'
                    ),
                    'grid'          => array(
                        'actionGroups'  => array(
                            'bulk'  => array(
    ...
    public static function setDefaultCommand(ResourceControllerCommand $command): void
    {
        if(empty($command->newResourceFactory))
        {
            $command->newResourceFactory = new CreateFichar();
        }
        if(empty($command->resource))
        {
            $command->resource = Fichar::class;
        }
        if(empty($command->command))
        {
            $command->command = new FicharCommand();
        }
    }

    public static function getResourceControllerClass(): string
    {
        return FicharController::class;
    }
